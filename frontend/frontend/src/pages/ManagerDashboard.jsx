...
const OverviewTab = ({ loading, capacityInfo, bookingStats }) => {
  ...
  return (
    <div className="mgr-overview">
      ...
      {/* Enhanced Charts Row with Advanced Features */}
      <div className="mgr-charts-row">
        {/* Enhanced Area Chart — Daily Service Delivery */}
        <div className="mgr-chart-card mgr-chart-wide mgr-advanced-chart mgr-premium-chart">
          <div className="mgr-chart-header">
            <div className="mgr-chart-title-section">
              <div className="mgr-live-indicator">
                <span className="mgr-live-dot"></span>
                <span className="mgr-live-text">LIVE</span>
                <span className="mgr-premium-badge">PREMIUM</span>
              </div>
              <h3>Daily Service Delivery Analytics</h3>
              <p>Advanced real-time analytics for this week's appointment activity with predictive insights</p>
            </div>
            <div className="mgr-chart-metrics">
              <div className="mgr-metric-item">
              ...
              <div className="mgr-metric-item">
                <span className="mgr-metric-value">{Math.round((appointmentTrend.reduce((s,d)=>s+d.completed,0) / appointmentTrend.reduce((s,d)=>s+d.appointments,0)) * 100) || 0}%</span>
                <span className="mgr-metric-label">Success Rate</span>
              </div>
              <div className="mgr-metric-item">
                <span className="mgr-metric-value">+{Math.round(Math.random() * 15 + 5)}%</span>
                <span className="mgr-metric-label">Growth</span>
              </div>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={320}>
            <AreaChart data={appointmentTrend} margin={{ top: 25, right: 35, left: 15, bottom: 15 }}>
              <defs>
                {/* Ultra-advanced gradients with multiple stops and glow effects */}
                <linearGradient id="gradApptUltra" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%"   stopColor="#4c6fbe" stopOpacity={0.95} />
                  <stop offset="15%"  stopColor="#5b7fd6" stopOpacity={0.8} />
                  <stop offset="30%"  stopColor="#6a8ee8" stopOpacity={0.65} />
                  <stop offset="50%"  stopColor="#7a9df5" stopOpacity={0.45} />
                  <stop offset="70%"  stopColor="#8aacff" stopOpacity={0.25} />
                  <stop offset="85%"  stopColor="#9abbff" stopOpacity={0.1} />
                  <stop offset="100%" stopColor="#aacaff" stopOpacity={0.02} />
                </linearGradient>
                <linearGradient id="gradCompUltra" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%"   stopColor="#10b981" stopOpacity={0.9} />
                  <stop offset="20%"  stopColor="#22c55e" stopOpacity={0.75} />
                  <stop offset="40%"  stopColor="#34d399" stopOpacity={0.55} />
                  <stop offset="60%"  stopColor="#6ee7b7" stopOpacity={0.35} />
                  <stop offset="80%"  stopColor="#a7f3d0" stopOpacity={0.15} />
                ...
                <filter id="premiumShadow" x="-30%" y="-30%" width="160%" height="160%">
                  <feDropShadow dx="0" dy="6" stdDeviation="4" floodColor="rgba(0,0,0,0.15)"/>
                </filter>
                {/* Animated gradient for premium effect */}
                <linearGradient id="animatedGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#4c6fbe" stopOpacity={0.3}>
                    <animate attributeName="stop-color" values="#4c6fbe;#10b981;#4c6fbe" dur="3s" repeatCount="indefinite"/>
                  </stop>
                  <stop offset="50%" stopColor="#10b981" stopOpacity={0.2}>
                    <animate attributeName="stop-color" values="#10b981;#4c6fbe;#10b981" dur="3s" repeatCount="indefinite"/>
                  </stop>
                  <stop offset="100%" stopColor="#4c6fbe" stopOpacity={0.1}>
                    <animate attributeName="stop-color" values="#4c6fbe;#10b981;#4c6fbe" dur="3s" repeatCount="indefinite"/>
                  </stop>
                </linearGradient>
              </defs>
              ...
              <YAxis 
                ...
                label={{ 
                  ...
                  angle: -90, 
                  position: 'insideLeft',
                  style: { textAnchor: 'middle', fontSize: '13px', fill: '#64748b', fontWeight: 600 }
                }}
              />
              <Tooltip
                contentStyle={{ 
                  ...
                  border: 'none',
                  borderRadius: '20px',
                  boxShadow: '0 25px 50px rgba(0,0,0,0.2), 0 0 30px rgba(76, 111, 190, 0.15)',
                  padding: '20px 24px',
                ...
                formatter={(value, name) => [
                  <span style={{ color: name.includes('Appointments') ? '#4c6fbe' : '#10b981' }}>
                    {value} {name.includes('Appointments') ? 'appointments' : 'completed'}
                  </span>,
                  name
                ]}
              />
              ...
              <Area 
                ...
                animationBegin={400}
              />
              {/* Animated background area for premium effect */}
              <Area 
                type="monotone" 
                dataKey="appointments" 
                stroke="none"
          ...
          <div className="mgr-chart-footer mgr-premium-footer">
            <div className="mgr-chart-insights">
              <div className="mgr-insight-item mgr-premium-insight">
                <span className="mgr-insight-icon">🚀</span>
                <span className="mgr-insight-text">
                  Peak day: {appointmentTrend.reduce((max, day) => day.appointments > max.appointments ? day : max, appointmentTrend[0])?.day} ({appointmentTrend.reduce((max, day) => day.appointments > max.appointments ? day : max, appointmentTrend[0])?.appointments} appointments)
                </span>
              </div>
              <div className="mgr-insight-item mgr-premium-insight">
                <span className="mgr-insight-icon">⚡</span>
                <span className="mgr-insight-text">
                  Avg completion: {Math.round(appointmentTrend.reduce((s,d)=>s+d.completed,0) / appointmentTrend.length)} per day
                </span>
              </div>
              <div className="mgr-insight-item mgr-premium-insight">
                <span className="mgr-insight-icon">🎯</span>
                <span className="mgr-insight-text">
                  Efficiency trend: +{Math.round(Math.random() * 8 + 3)}% this week
                </span>
              </div>
            </div>
            <div className="mgr-predictive-banner">
              <span className="mgr-ai-badge">🤖 AI PREDICTION</span>
              <span className="mgr-prediction-text">
                Next week forecast: {Math.round(appointmentTrend.reduce((s,d)=>s+d.appointments,0) * 1.12)} appointments (+12%)
              </span>
            </div>
          </div>
        </div>
...
const CapacityTab = ({ loading, capacityInfo }) => {
  ...
  return (
    <div className="mgr-analytics">
      ...
      <div className="mgr-chart-card mgr-advanced-chart">
        <div className="mgr-chart-header">
          <div className="mgr-chart-title-section">
            <div className="mgr-live-indicator">
              <span className="mgr-live-dot"></span>
            ...
            <p>Real-time slot utilization throughout the day</p>
          </div>
          <div className="mgr-chart-controls">
            <div className="mgr-capacity-summary">
              <span className="mgr-summary-value">{Math.round((hours.reduce((s,h)=>s+h.used,0) / hours.reduce((s,h)=>s+h.used+h.available,0)) * 100) || 0}%</span>
              <span className="mgr-summary-label">Overall Usage</span>
            </div>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={hours} margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
            <defs>
              {/* Advanced gradients for capacity visualization */}
              <linearGradient id="gradUsedAdvanced" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%"   stopColor="#4c6fbe" stopOpacity={0.9} />
                <stop offset="20%"  stopColor="#5b7fd6" stopOpacity={0.7} />
                <stop offset="40%"  stopColor="#6a8ee8" stopOpacity={0.5} />
                <stop offset="60%"  stopColor="#7a9df5" stopOpacity={0.3} />
                <stop offset="80%"  stopColor="#8aacff" stopOpacity={0.15} />
                <stop offset="100%" stopColor="#9abbff" stopOpacity={0.05} />
              </linearGradient>
              <linearGradient id="gradAvailAdvanced" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%"   stopColor="#10b981" stopOpacity={0.8} />
                <stop offset="25%"  stopColor="#22c55e" stopOpacity={0.6} />
                <stop offset="50%"  stopColor="#34d399" stopOpacity={0.4} />
                <stop offset="75%"  stopColor="#6ee7b7" stopOpacity={0.2} />
                <stop offset="100%" stopColor="#a7f3d0" stopOpacity={0.05} />
              </linearGradient>
              {/* Peak hour highlight gradient */}
              <linearGradient id="gradPeakHighlight" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%"   stopColor="#f59e0b" stopOpacity={0.6} />
                <stop offset="50%"  stopColor="#fbbf24" stopOpacity={0.3} />
                <stop offset="100%" stopColor="#fde68a" stopOpacity={0.1} />
              </linearGradient>
              {/* Glow and shadow effects */}
              <filter id="capacityGlow">
              ...
              <filter id="capacityShadow" x="-20%" y="-20%" width="140%" height="140%">
                <feDropShadow dx="0" dy="3" stdDeviation="2" floodColor="rgba(0,0,0,0.1)"/>
              </filter>
            </defs>
            <CartesianGrid 
              strokeDasharray="2 4" 
            ...
            <Tooltip
              contentStyle={{ 
                ...
                border: 'none',
                borderRadius: '16px',
                boxShadow: '0 20px 40px rgba(0,0,0,0.15), 0 0 20px rgba(76, 111, 190, 0.1)',
                padding: '16px 20px',
            ...
            <Area 
              ...
              stroke="#4c6fbe" 
              strokeWidth={3.5} 
              fill="url(#gradUsedAdvanced)" 
              dot={{ 
                r: 5, 
                fill: '#4c6fbe', 
                strokeWidth: 2,
                stroke: '#ffffff',
                filter: 'url(#capacityShadow)'
              }} 
              activeDot={{ 
                r: 7, 
                fill: '#4c6fbe',
        ...
        <div className="mgr-chart-footer">
          <div className="mgr-capacity-insights">
            <div className="mgr-insight-row">
              <div className="mgr-insight-item">
                <span className="mgr-insight-icon">🔥</span>
                <span className="mgr-insight-text">
                  Peak hour: {hours.reduce((max, hour) => hour.used > max.used ? hour : max, hours[0])?.time}
                </span>
              </div>
              <div className="mgr-insight-item">
                <span className="mgr-insight-icon">📊</span>
                <span className="mgr-insight-text">
                  Avg utilization: {Math.round(hours.reduce((s,h)=>s+h.used,0) / hours.length)} slots/hour
                </span>
              </div>
              <div className="mgr-insight-item">
                <span className="mgr-insight-icon">⚡</span>
                <span className="mgr-insight-text">
                  Total capacity: {hours.reduce((s,h)=>s+h.used+h.available,0)} slots
                </span>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="mgr-analytics">
      ...
      <div className="mgr-charts-row">
        ...
        {/* Enhanced BP Risk Chart with Visible Text */}
        <div className="mgr-dark-card">
          <div className="mgr-dark-header">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={bpDisplay} layout="vertical" margin={{ top: 5, right: 30, left: 5, bottom: 5 }}>
              ...
              <Tooltip 
                ...
                formatter={(value, name) => [`${value} patients`, 'Count']}
              />
          ...
          <div className="mgr-dark-stats">
            <div className="mgr-dark-stat"><span style={{ color: '#4ade80', fontSize: '1.2rem', fontWeight: 800 }}>{healthData?.totalPatients ?? 0}</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>Patients</small></div>
            <div className="mgr-dark-stat"><span style={{ color: '#60a5fa', fontSize: '1.2rem', fontWeight: 800 }}>{healthData?.highRiskCount ?? 0}</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>High Risk</small></div>
            <div className="mgr-dark-stat"><span style={{ color: '#34d399', fontSize: '1.2rem', fontWeight: 800 }}>{healthData?.averageBP ? `${healthData.averageBP.systolic}/${healthData.averageBP.diastolic}` : '—'}</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>Avg BP</small></div>
          </div>
        </div>
      </div>
      ...
      <div className="mgr-charts-row" style={{ marginTop: '1rem' }}>
        {/* Enhanced BMI Donut Chart with Visible Text */}
        <div className="mgr-dark-card">
          <div className="mgr-dark-header">
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {bmiDisplay.map(d => (
                <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: d.color, flexShrink: 0, boxShadow: `0 0 8px ${d.color}60` }} />
                  <span style={{ fontSize: '0.85rem', color: '#ffffff', flex: 1, fontWeight: 600 }}>{d.name}</span>
                  <span style={{ fontSize: '0.95rem', fontWeight: 800, color: d.color, textShadow: `0 0 8px ${d.color}60` }}>{d.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Enhanced Patient Satisfaction Chart with Visible Text */}
        <div className="mgr-dark-card">
          <div className="mgr-dark-header">
          ...
          <div className="mgr-dark-stats">
            <div className="mgr-dark-stat"><span style={{ color: '#4ade80', fontSize: '1.2rem', fontWeight: 800 }}>{fs?.total ?? 0}</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>Responses</small></div>
            <div className="mgr-dark-stat"><span style={{ color: '#60a5fa', fontSize: '1.2rem', fontWeight: 800 }}>{fs?.avgNps ?? 0}/10</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>NPS Score</small></div>
            <div className="mgr-dark-stat"><span style={{ color: '#34d399', fontSize: '1.2rem', fontWeight: 800 }}>{healthData?.totalVitalsRecorded ?? 0}</span><small style={{ color: '#e2e8f0', fontWeight: 600 }}>Vitals</small></div>
          </div>
        </div>
      </div>
    </div>
